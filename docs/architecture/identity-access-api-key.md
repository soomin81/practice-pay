...
### 4.6 내부 운영자의 가맹점 계정 관리

가맹점이 스스로 잠기거나(예: 마지막 활성 OWNER가 정지됨) 계정 사고가 났을 때 PG가
개입할 수 있어야 한다. 가맹점 사용자 계정 관리(정지·재개·종료·역할 변경)는 가맹점
콘솔(OWNER/ADMIN)뿐 아니라 **내부 운영자 콘솔에서도** 할 수 있다.

- **관리 행위는 `SUPER_ADMIN`과 `OPERATOR`가 할 수 있다.** `SUPER_ADMIN`은 "전체 관리",
  `OPERATOR`는 "가맹점·결제·운영 업무"(3.2)에 해당한다 — 가맹점 등록(최초 OWNER 생성,
  6.6과 같은 스코핑)을 이미 이 두 역할이 하므로, 그 가맹점의 사용자 관리도 같은 역할
  집합에 두는 것이 일관적이다. `VIEWER`는 조회 전용이라 명부만 볼 수 있다.
- **"가맹점에는 최소 하나의 활성 OWNER가 항상 존재해야 한다"(4.4)가 여기서 강제된다** —
  마지막 활성 OWNER를 정지·종료·강등하려는 요청은 거부한다. 가맹점 콘솔 경로에서는 요청자
  자기 자신 차단·`ADMIN`→`OWNER` 제한 때문에 이 불변식이 실제로 트리거되지 않지만, 내부
  운영자 경로에는 그 제약이 없어 이 규칙이 실제로 작동하는 첫 경로다.
- 내부 운영자는 가맹점의 초대를 대신 발급·재발송·취소하지 않는다 — 그건 가맹점
  OWNER/ADMIN의 몫이다.
...
**내부 운영자·가맹점 관리자 로그인은 각각 별도 감사 로그로 확장됐다**(`internal_login_audit`,
`merchant_login_audit`) — 집계 필드(`last_login_at`)와 달리 시도 하나하나(성공·실패·잠김, 없는
loginId/merchantCode 시도 포함)를 클라이언트 IP와 함께 append-only로 남긴다. 내부 로그인은
`AuthenticateInternalUserUseCase`(api-admin)가 기록해 `GET /admin/login-audit`(SUPER_ADMIN
전용)로, 가맹점 로그인은 `AuthenticateMerchantUserUseCase`(api-merchant)가 기록해
`GET /admin/merchant-login-audit`(SUPER_ADMIN/OPERATOR, 전 가맹점)로 조회한다. **API Key 사용
감사**는 같은 인프라를 확장하는 후속이다.
...
- 가맹점 사용자 로그인
- 내부 운영자(SUPER_ADMIN/OPERATOR)의 가맹점 계정 관리(정지·재개·종료·역할 변경, 4.6)
- TEST API Key 복수 발급
- API Key 최초 1회 표시
- API Key 해시 저장
- API Key 폐기
...
- 내부 운영자·가맹점 관리자 로그인 감사 로그 기록·조회(8절, `GET /admin/login-audit`·`GET /admin/merchant-login-audit`)
...
- 계정 승인 워크플로
- 세분화된 RBAC
- API Key 사용 감사 조회(내부·가맹점 로그인 감사는 MVP에 포함됨 — 8절)
