...
**이 표가 오류 응답의 기준이다 — 생성되는 OpenAPI 스펙에는 오류가 없다.** 스펙은 통과한
`@WebMvcTest`에서 뽑는데, MockMvc가 컨테이너의 ERROR 디스패치를 재현하지 않아 4xx를
잘못 적을 위험이 있어서 의도적으로 뺐다(`backend/CLAUDE.md`의 "테스트가 잡지 못하는 층").
위 값들은 실제 `bootRun` + `curl`로 확인했다.
