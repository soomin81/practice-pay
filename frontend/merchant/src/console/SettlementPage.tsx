	...
	return (
		<>
			...
			{/* **여기 두 값은 목록 API가 실제로 내려주는 것뿐이다**(`totalCount`,
			    `totalNetAmount`, `heldCount`/`heldNetAmount`). 없는 값을 그럴듯하게 만들면 그
			    숫자를 아무도 설명할 수 없게 된다.

			    보류 금액을 합계 옆에 두는 이유: 합계는 지급 경로에 살아 있는 것만 더하므로
			    막힌 돈이 있으면 그만큼 작아진다 — 그 차이를 설명하는 값이 같은 줄에 없으면
			    "어제보다 왜 줄었나"에 답할 수 없다. **이 콘솔에는 푸는 수단이 없다**(PG가 한다). */}
			<div className="pb-6">
				<StatStrip
					stats={[
						...
						},
						{
							...
							value: settlements.data
								? `${formatKrw(settlements.data.heldNetAmount)} (${heldCount.toLocaleString('ko-KR')}건)`
								: '—',
						},
					]}
				/>
