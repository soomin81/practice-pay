...
import { merchantApi, MerchantApiError } from '@/api/client'
...
export function usePaymentExport() {
	return useMutation({
		mutationFn: async (filters: PaymentListFilters) => {
			const file = await merchantApi.exportPayments(filters)
...
export function exportErrorMessage(error: unknown): string {
	return error instanceof MerchantApiError ? error.message : '엑셀 파일을 만들지 못했습니다.'
}
