...
/**
 * 화면 위쪽의 통계 네 칸. **전부 필터 전체에 대한 값**이고 현재 페이지와 무관하다.
 *
 * 승인율은 서버에서 받지 않고 여기서 나눈다 — 두 건수를 이미 받았으므로 서버가 한 번 더
 * 계산할 이유가 없고, 반올림 방식을 화면이 정하는 편이 낫다.
 *
 * **분모가 0이면 "—"다.** `0/0`을 0%로 그리면 "결제가 없다"와 "전부 실패했다"가 화면에서
 * 같아진다.
 */
