...
/**
 * 초대 수락(계정 활성화) 화면. **인증 게이트 밖에 있다** — 초대받은 사람은 아직 계정이
 * 활성화되지 않아 로그인할 수 없다(`App.tsx`의 라우트 주석 참고).
 *
 * Token은 쿼리스트링으로 받는다(`/accept-invitation?token=…`) — 발급 화면이 만들어 주는
 * 링크 형식이다. 성공해도 로그인 상태가 되지는 않으므로 로그인 화면으로 안내한다.
 */
		...
		<div className="flex min-h-dvh items-center justify-center p-6">
			<Card className="w-full max-w-sm">
				<CardHeader>
					...
					<CardDescription>초대받은 계정의 비밀번호를 설정합니다.</CardDescription>
				</CardHeader>
				<CardContent>
					{!token ? (
...
function acceptErrorMessage(error: unknown): string {
	if (error instanceof MerchantApiError) {
		// 400은 유효하지 않거나 만료·이미 사용된 초대다 — 서버가 원인을 일부러 구분해 주지 않는다.
