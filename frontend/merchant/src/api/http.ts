 ...
 *
 * **admin 콘솔의 같은 파일과 거의 동일하다** — 두 앱이 워크스페이스를 쓰지 않는 독립
 ...
 *
 * 각 앱은 이 클래스를 그대로 쓰지 않고 자기 이름의 하위 클래스를 노출한다(`MerchantApiError`) —
...
export interface HttpConfig {
	/** API 서버 주소(예: `http://localhost:8083`). */
	baseUrl: string
	/**
	...
	/** 상태 코드와 메시지로 앱 고유 오류를 만든다(`MerchantApiError` 등). */
