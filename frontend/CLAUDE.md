...
- **지갑 등록은 `connect` 동작이 아니라 `session.connectedWallet`을 보고 한다.** wagmi가 연결 상태를 브라우저에 저장하므로, 새 체크아웃 세션에서는 화면이 연결 버튼을 건너뛰고 곧장 "보내기"를 보여준다 — 등록을 연결 동작에만 맡기면 **그 세션은 지갑이 등록된 적이 없는 채로 전송 서명까지 가고**, 제출이 "연결된 지갑이 없습니다"로 거부된다(**돈은 나가고 결제는 실패**). 실물 검증에서 실제로 겪었고 `walletRegistration.test.ts`가 회귀로 지킨다.
- **주소는 `asAddress`가 viem `getAddress`로 정규화한다.** 백엔드는 EIP-55 체크섬을 검증하지 않아(의도적) 수취 지갑이 소문자로 내려올 수 있는데, 정규화하지 않으면 viem이 인코딩 단계에서 거부해 결제가 지갑 단계에서 막힌다.
- **DEV 결제 생성은 Vite 프록시(`/api/v1` → `:8081`)를 탄다.** 백엔드는 CORS를 `/checkout/**`에만 열어 두는데(보안 경계), DEV 버튼이 부르는 결제 생성 API는 그 밖이라 브라우저에서 직접 부르면 차단된다. 프록시는 개발 서버에만 있어 백엔드 경계를 넓히지 않는다 — **체크아웃 호출은 일부러 프록시를 태우지 않는다**(운영에서도 교차 출처라 개발 중에도 진짜 CORS를 겪어야 설정이 깨진 것을 발견한다).
...
- **결제 완료·취소 후의 복귀 화면도 이 앱이 갖는다**(`?dev-return=success|cancel` → `DevMerchantReturn`). 원래 그 자리는 가맹점의 자기 사이트인데 로컬에는 없어서 예전에는 `merchant.example.com`을 가리켰고, **마지막 단계가 죽은 도메인으로 끝나** 흐름을 끝까지 확인할 수 없었다. DEV 전용이라 운영 번들에서 제거되는 것을 빌드 산출물로 확인했다. 화면에 "가맹점 사이트를 대신하는 자리"임과 **실제 가맹점은 이 화면이 아니라 Webhook으로 주문을 확정해야 한다**는 것(복귀와 Webhook의 순서는 보장되지 않는다 — 계약 8절)을 함께 적어 둔다.
...
- **DEV 전용 "테스트 결제 생성" 버튼**이 가맹점 서버 역할을 대신한다(`src/dev/`). 체크아웃은 `checkoutSessionId`가 있어야 시작하는데 그건 API Key가 필요한 `POST /api/v1/payments`가 만들기 때문이다.
  - **API Key는 `.env.local`에서만 읽는다**(`.env.example` 복사). 없으면 버튼이 꺼지고 안내만 뜬다 — **코드에 기본값을 두지 않는다.**
  - **수취 지갑은 이 버튼이 보내지 않는다.** PG가 수탁하는 지갑이라 백엔드 설정(`APP_PAYMENT_RECEIVING_WALLETS_BASE_SEPOLIA`)에서만 온다 — 프론트가 지정할 수 있으면 가맹점 역할이 USDC를 직접 받으면서 정산 채권까지 받는다([`docs/architecture/mvp-scope.md`](../docs/architecture/mvp-scope.md)의 "수취 지갑 귀속"). 한때 `VITE_DEV_RECEIVING_WALLET`으로 여기서 보냈고, 그 전에는 USDC 토큰 Contract 주소가 기본값으로 하드코딩돼 있기까지 했다(복붙 사고 — 그리로 보낸 토큰은 되찾을 수 없다). **요청 본문에 `receivingWallet`이 없다는 것을 `DevPaymentCreator.test.tsx`가 회귀로 지킨다.** 백엔드 설정이 없으면 버튼이 503을 받는다 — 준비 절차는 [`docs/guides/testnet-wallet-setup.md`](../docs/guides/testnet-wallet-setup.md).
...
- **가맹점 콘솔에도 같은 화면이 있다**(가맹점 열만 없는 복제본). 그쪽은 서버가 `merchantId`를
  인증 주체에서 받아 **조회한 결제가 그 가맹점 것인지 확인**하고, **없는 결제와 남의 결제를
  똑같이 404로** 돌려준다 — 단건 조회는 "필터가 비면 전체가 나온다"가 아니라 **"남의 것을
  ID로 찍어 볼 수 있다"**는 형태로 새기 때문이다. 화면도 둘을 구분해 보여주지 않는다.
  **한쪽을 고치면 다른 쪽도 함께 본다.**
