...
/**
 * 결제 상세 문서화에 쓰는 픽스처와 필드 서술이다. 응답의 중첩이 깊어 서술이 70줄을 넘는데,
 * `AdminApiDocumentationTest` 본문에 두면 나머지 엔드포인트의 문서화가 그 사이에 묻힌다.
 */
private val DOC_NOW: Instant = Instant.parse("2026-08-01T04:07:24Z")

/**
 * **모든 단계가 채워진 결제**를 쓴다(실물 검증에서 나온 값과 같은 모양) — REST Docs는 값이
 * `null`인 필드의 타입을 추론하지 못해 스펙에서 조용히 빠뜨린다(이미 `failureReason`에서
 * 한 번 겪었다). 그래서 nullable 필드에는 `.type(...)`도 함께 명시한다.
 */
internal fun paymentDetailFixture() =
	PaymentDetailView(
		payment =
...
/**
 * **REST Docs는 문서화되지 않은 응답 필드가 있으면 실패한다** — 그 강제 덕분에 스펙이 실제
 * 응답과 어긋날 수 없다. 그래서 중첩된 필드까지 전부 적는다.
 */
internal fun paymentDetailFields(): List<FieldDescriptor> =
	paymentFields() + quoteFields() + checkoutFields() + blockchainFields() + exchangeFields() + settlementFields() + webhookFields()

private fun paymentFields() =
	listOf(
		// 부모 객체도 선언해야 스펙에서 required로 나온다 — 빠뜨리면 생성된 타입이
		// optional이 되어 화면 코드가 매번 null 검사를 하게 된다(실제로 그렇게 됐다).
		fieldWithPath("payment").description("결제 본문. 언제나 있다."),
		fieldWithPath("payment.paymentId").description("결제 식별자"),
		fieldWithPath("payment.merchantId").description("가맹점 식별자"),
		fieldWithPath("payment.merchantName").description("가맹점 이름"),
...
private fun quoteFields() =
	listOf(
		fieldWithPath("quote").description("견적 스냅샷. 결제 생성 트랜잭션에서 함께 만들어져 언제나 있다."),
		fieldWithPath("quote.marketProviderCode").description("시장 환율 제공자 코드"),
		fieldWithPath("quote.marketRate").description("시장 환율"),
		fieldWithPath("quote.appliedRate").description("적용 환율(시장 환율에 스프레드를 적용한 값)"),
...
private fun checkoutFields() =
	listOf(
		fieldWithPath("checkoutSession").description("체크아웃 세션. 결제 생성과 함께 만들어져 언제나 있다."),
...
private fun blockchainFields() =
	listOf(
		...
		fieldWithPath("blockchainTransaction.amountMinor")
			.type(JsonFieldType.STRING)
...
private fun webhookFields() =
	listOf(
		fieldWithPath("webhookDeliveries").description("Webhook 전송 이력(오래된 순). 가맹점이 설정하지 않았으면 빈 배열."),
		fieldWithPath("webhookDeliveries[].webhookDeliveryId").description("전송 식별자"),
		fieldWithPath("webhookDeliveries[].eventType").description("이벤트 종류(payment.created 등)"),
		...
		fieldWithPath("webhookDeliveries[].lastHttpStatus")
			.type(JsonFieldType.NUMBER)
		...
		fieldWithPath("webhookDeliveries[].nextRetryAt")
			.type(JsonFieldType.STRING)
