...
apps/
  ...
  api-admin/         실제 Gradle 서브프로젝트, 독립 배포 가능한 Spring Boot 앱 — webmvc + jooq + security,
                     ...
                     AdminListMerchantUsersUseCase/AdminChangeMerchantUserStatusUseCase/
                     AdminChangeMerchantUserRoleUseCase(GET·POST
                     /admin/merchants/{merchantId}/users/... — 내부 운영자가 임의 가맹점의
                     사용자를 조회·관리한다. 조회는 전원, 변경은 SUPER_ADMIN/OPERATOR.
                     "최소 하나의 활성 OWNER를 유지한다" 불변식이 실제 HTTP로 트리거되는
                     첫 경로다 — IMPLEMENTATION-NOTES 참고),
...
- **`test("...")`의 이름은 영문으로 쓴다.** KDoc·검증 메시지·커밋 메시지를 한글로 쓰는 것과 반대라 헷갈리기 쉬운데, 백엔드 테스트 157개 파일이 전부 영문이라 다수파를 기준으로 고정한다(`frontend/`는 정반대로 **한글**이 다수파다 — 티어마다 규칙이 다르니 작업 중인 쪽을 따른다). 이름은 동작을 서술하고(`refuses a blank note`), *왜* 그 동작이 맞는지는 테스트 위 KDoc이나 인라인 주석에 **한글로** 적는다.
...
- **공개 ID(`*_id`) ↔ 내부 PK(`*_seq`) 변환도 `infra.persistence.jooq.SeqResolution.kt`의 공유 `DSLContext` 확장(`merchantSeq(id)`/`merchantId(seq)`/`internalUserSeq`/`merchantUserSeq`/`paymentSeq`/`exchangeOrderSeq` 등)을 거친다** — Adapter마다 `resolveMerchantSeq`류 private 함수로 같은 조회를 복제하지 않는다(원래 `resolve*` 33개가 12개 어댑터에 흩어져 있던 것을 모았다). `InstantMapping`과 같은 이유·같은 위치(하위 패키지 어댑터는 이 확장을 import해서 쓴다). 없는 값이면 `error(...)`로 즉시 실패한다 — FK 대상 존재는 호출부가 이미 보장한 상태라 부재는 정합성 깨짐이다.
...
- **동시 쓰기 보호는 `version` 재조회가 아니라 행 잠금(`SELECT ... FOR UPDATE`)이 담당한다.** 도메인 애그리게이트는 `version` 필드를 갖고 있지 않아서(영속성 관심사를 도메인에 새지 않으려고 의도적으로 뺐다) `save()`는 UPDATE 직전에 DB의 현재 `version`을 다시 읽어 `current + 1`을 쓴다 — 이건 "이 애그리게이트가 오래된 version에서 읽혔다"를 잡지 못한다(트랜잭션 밖 경로에서는 조용한 lost update가 되고, 트랜잭션 안에서는 시끄러운 실패가 된다). 그래서 **경합이 실제로 일어나는 세 애그리게이트**(`Payment`/`CheckoutSession`/`MerchantApiKey`)는 Port에 `findByIdForUpdate`(API Key는 `findByPrefixForUpdate`)를 두고, **변경할 목적의 읽기를 트랜잭션 안에서 잠근 채** 수행한다.
  - **잠금은 로드 시점에 잡아야 한다** — `save()` 안에서 잠그는 것으로는 못 막는다(문제의 읽기는 그 앞의 `findById`다). 그래서 `ConnectCheckoutWalletUseCase`/`CancelCheckoutSessionUseCase`/`AuthenticateApiKeyUseCase`는 저장을 묶기 위해서가 아니라 **잠금을 저장까지 유지하려고** `TransactionManager`를 쓴다.
  - **잠금 순서는 `Payment` → `CheckoutSession`으로 통일한다**(반대로 잠그는 경로가 생기면 교착이 난다). `SubmitPaymentTransactionUseCase`가 세션을 한 번 잠그지 않고 읽는 이유가 이것이다 — `paymentId`를 얻어 Payment부터 잠그기 위해서다.
  - **`ConfirmBlockchainTransactionUseCase`는 예외로 두었다** — 중간에 온체인 RPC 호출이 있어 잠그면 네트워크 지연 동안 행을 붙잡게 된다. 이 Use Case가 다루는 Payment는 이미 `PROCESSING`/`CONFIRMING`이라 만료 Sweep(`CREATED`/`READY`만 선택)·제출과 겹치지 않아 경합 창이 사실상 없다.
  - **나머지 7개 어댑터는 여전히 재조회 방식이다** — 같은 패턴이지만 두 흐름이 같은 행을 동시에 바꾸는 경로가 없다. 그런 경로가 생기면 같은 방식(잠금 조회 + 트랜잭션)으로 넓힌다.
  - 회귀는 `PaymentConcurrentWriteTest`/`MerchantApiKeyConcurrentWriteTest`(Testcontainers, 두 스레드)가 지킨다 — 후자는 **폐기가 되돌려지지 않는다**를 고정하는 보안 성격의 테스트다.
...
- **Webhook 전송(`webhook`)과 서명(`webhooksignature`)을 굳이 나눈 이유**는 위 규칙의 대표 사례다: 서명 쪽만 `@Value`로 Pepper를 요구하고, `api-merchant`는 **비밀을 보여주기만 할 뿐 Webhook을 보내지 않는다**. 한 패키지로 합치면 `api-merchant`가 쓰지도 않는 `HttpWebhookSender`까지 끌고 온다.
