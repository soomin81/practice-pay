...
private fun initialOwner(): MerchantUser = MerchantUser.inviteInitialOwner(
...
class MerchantUserTest : FunSpec({
	...
	test("inviteSubAccount creates an INVITED ADMIN invited by a merchant user") {
		val admin = MerchantUser.inviteSubAccount(
	...
	test("canInviteSubAccounts and canManageApiKeys are true only for ACTIVE OWNER/ADMIN") {
		...
		val viewer = MerchantUser.inviteSubAccount(
