...
### 실물 검증 — 두 앱을 걸쳐 원문이 왕복하는 것을 확인했다

**암호화는 `api-payment`가, 복호화는 `api-admin`이** 해야 의미가 있으므로 두 앱을 함께 띄우고
같은 DB로 21가지를 확인했다. 권한 경계를 보려고 OPERATOR·VIEWER 계정을 임시로 심었다.

| 확인 | 결과 |
|---|---|
| 검색(SUPER_ADMIN·OPERATOR) | 200, 마스킹 값과 조인해 온 가맹점·주문 정보가 함께 온다 |
| **대소문자가 다른 이메일로 검색** | 찾는다 — 정규화가 저장·검색 양쪽에서 같게 계산된다 |
| 검색(VIEWER) | 403 |
| 원본 열람(SUPER_ADMIN) | 200, `api-payment`가 암호화한 값이 그대로 복원된다 |
| **원본 열람(OPERATOR)** | **403** — 이 슬라이스에서 가장 중요한 경계다 |
| 빈 사유 / 조건 둘 다 / 조건 없음 / 잘못된 이메일 | 전부 400 |
| **없는 결제로 열람** | **404** — 앞에서 막지 않았으면 500이 나올 자리다 |
| 결제는 있고 구매자 정보만 없음 | 404(위와 구분하지 않는다) |
| 없는 이메일로 검색 | 200 + 빈 배열 |
| 감사 기록 | 열람자·역할·결제·사유·IP·시각이 남고, **응답의 `revealedAt`과 같은 시각**이다 |
| 감사 기록에 평문 | 0건 |
| `api-admin` 로그에 평문 | 0건(응답에는 나갔지만 로그에는 없다) |

### 키가 어긋나면 **검색은 멀쩡하고 열람만 터진다**

`--app.pii.encryption-key`를 다른 값으로 주고 다시 띄워 확인했다.

- 검색은 **200으로 정상 동작한다** — `*_masked` 컬럼만 읽어 복호화를 타지 않기 때문이다.
- 열람은 500이고, WARN이 원인을 짚는다: "app.pii.encryption-key가 암호화할 때와
  다르거나(api-payment/api-admin 불일치)". 예외만 보면 `AEADBadTagException: Tag mismatch`뿐이라
  **설정 문제라는 것을 알 길이 없다** — 이 로그를 넣은 이유가 그대로 증명됐다.
- 이 조합이 특히 나쁘다: 운영자에게는 "검색은 되는데 열람만 안 된다"로 보여서 데이터가 깨진
  것처럼 읽힌다. 실제로는 설정 하나가 어긋난 것이다.

### 검증하다 드러난 것 둘

- **거부된 열람 시도는 기록되지 않는다.** OPERATOR가 열람을 시도해 403을 받아도
  `customer_pii_access_audit`에는 아무것도 남지 않는다(성공 1건, 시도 5건 → 기록 1건).
  `internal_login_audit`이 실패·잠김까지 남기는 것과 **비대칭**이다. 지금 구조에서는 인가가
  `SecurityConfig`에서 끝나 Use Case까지 오지 않아서 기록할 자리 자체가 없다 — 남기려면
  인가 실패 지점에 훅을 걸어야 한다. 후속으로 판단한다.
- **검증용 계정을 지울 때 로그인 감사 FK가 막았다.** 감사 기록이 계정보다 오래 살아야 한다는
  설계가 실제로 동작하는 것이라 정상이다 — 지우려면 감사 행을 먼저 지워야 한다.

검증에 쓴 결제·구매자 정보·감사 기록과 임시 계정 둘은 전부 지웠다.
...
- **열람 감사를 읽는 경로가 없다.** `customer_pii_access_audit`에 쌓이기만 한다 — 로그인
  감사와 같은 모양이면 된다.
- **관리자 화면(`frontend/admin`)이 없다.** 백엔드만 붙었고 실물 검증도 `curl`까지다.
- **거부된 열람 시도가 기록되지 않는다**(위 참고) — 로그인 감사와 비대칭이다.
